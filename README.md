...
- **图片离线缓存**：收藏/浏览的图片以 Blob 存入 IndexedDB，离线时优先本地读取（双层兜底 SW 运行时缓存）
- **i18n**：中文 / English 一键切换
- **深色模式**：浅色/深色主题
- **数据自动更新**：GitHub Actions 每周日定时重跑开放数据管道，仅变更时自动提交并重新部署
...
- **迭代 6 — 打磨发布**：响应式细调、部署、品牌命名、原生扩展
- **迭代 7 — 离线增强 + 数据自更新**：图片 IndexedDB 离线缓存（双层兜底）+ 每周自动重跑数据管道 ✅
